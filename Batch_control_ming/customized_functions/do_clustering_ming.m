% do_clustering_ming: modified from do_clustering

function [cluster_class, spikes, par, inspk, ipermut] = do_clustering_ming(data, spikes, index, filename)
...
subplot(3,1,1)
box off; hold on
suptitle(strrep(file_to_cluster, '_', ' ')); % replace underscore with space for display purpose
...
allfig_h = get(0,'children');  % save a figure for each channel
for i = 1 : length(allfig_h)
  saveas(allfig_h(i), [file_to_cluster,  '_', 'Fig', num2str(i)], 'jpg');
  pause(1);
end
close all;
