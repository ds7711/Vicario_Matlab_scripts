% Get_Spike_Ming: modified from get_spikes

function [index, spikes] = get_spikes_ming(filename, data)
% PROGRAM Get_spikes.
...
handles.par.sr = 25000;                     %sampling frequency, in Hz (default 24000).
...
%     eval(['load ' char(file_to_cluster) ';']);
    ...
    index_all = [index_all index];          % spikes timing expressed in terms of index of the factor
    spikes_all = [spikes_all; spikes];
end
index = index_all *1e3/handles.par.sr;                  %spike times in ms.
spikes = spikes_all;
eval(['save ', filename, '_spikes.mat spikes index']);    %saves Sc files
digits = round(handles.par.stdmin * 100);
nfile = [filename '_sp_th.mat' num2str(digits)];
